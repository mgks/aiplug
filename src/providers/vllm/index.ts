/**
 * Auto-generated thin instance for `vllm`.
...
export class VllmTransport extends OpenAICompatibleTransport {
  constructor(config: ProviderConfig) {
    if (!config.baseURL) {
      throw makeError({
        code: 'INVALID_CONFIGURATION',
        transport: 'vllm',
        message: 'Transport "vllm" requires a baseURL',
      });
    }
    super(config);
  }
  ...
  override capabilities(): TransportMetadata {
    ...
    return { ...m, name: 'vllm', defaultBaseURL: 'http://localhost:8000/v1' };
  }
}
